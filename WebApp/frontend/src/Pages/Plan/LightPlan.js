...
const LightPlan = () => {
  ...
  const handleSubmit = (e) => {
    ...
    let day =
      moment(data.date.value).format("YYYY-MM-DD") + " " + data.timeF.value;
  ...
  const handleDelete = () => {
    dispatch(
      deleteSched({ id: ids, userName: sessionStorage.getItem("userName") })
    );
  };
  // Handle change date:
  const handle_date = (e) => {
    if (
      moment(e.currentTarget.value).isAfter(moment(time).format("YYYY-MM-DD"))
    ) {
      SetCurr(false);
    } else SetCurr(true);
  };
  const handle_date_edit = (e) => {
    ...
    if (
      moment(e.currentTarget.value).isAfter(
  ...
  const totalLight = Number(useSelector((state) => state.schedule.totalLight));
  const countLightPage = Math.ceil(totalLight / 7);
  const setPages = (e, p) => {
    setLightPage(p);
  };
  useEffect(() => {
    dispatch(getLight(lightPage));
  });
  return (
    <>
      {show && (
        ...
        >
          <Form onSubmit={handleSubmit}>
            <Modal.Header closeButton>
              <Modal.Title>
                <h4>Choose time for turning the light on</h4>
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
              <div className="form-group">
                ...
                <input
                  ...
                  min={moment(time).format("YYYY-MM-DD")}
                  defaultValue={moment(time).format("YYYY-MM-DD")}
                  name="date"
              ...
              <div className="form-group">
                <label htmlFor="timeF" className="col-form-label">
                  Choose time
                </label>
                <input
                  type="time"
                  min={
                    curr
                      ? moment(time).add(1, "minutes").format("HH:mm")
                      : "00:00"
                  }
            ...
            <Modal.Footer>
              <Button
                variant="secondary"
                className="border font-weight-normal px-5"
                onClick={handleClose}
              >
                Huỷ
              </Button>
            ...
            <Modal.Body>
              <div className="form-group">
                ...
                <input
                  ...
                  defaultValue={moment(curData.time).format("YYYY-MM-DD")}
                  name="date"
                  required
                />
                <div className="invalid-feedback">Choose a date</div>
              ...
              <div className="form-group">
                ...
                <input
                  ...
                  defaultValue={moment(curData.time).format("HH:mm")}
                  className="form-control"
                  id="timeF"
                  required
                />
      ...
      <div className="container-fluid p-0 p-md-4 d-flex flex-column w-100 gap-2">
        <div
          className="row border border-gray-300 rounded bg-white p-3 mx-2 flex-row w-100 justify-content-between align-items-center"
          id="top-nav"
        >
          <h6 className="col m-0">Light schedule</h6>
          <h6 className="col m-0 text-end">{rec}</h6>
        </div>
        <div
          className="row border border-gray-300 rounded bg-white p-3 mx-2 flex-row w-100 h-100 justify-content-between align-items-center"
          id="tool"
        >
          ...
          <div className="row d-flex flex-sm-row flex-column w-100 justify-content-between align-items-center mt-2  gap-1">
            ...
            >
              <p style={{ color: "#6C757D" }}>
                Hiển thị {light.length} trong tổng {totalLight} dữ liệu
              </p>
            </div>
            <div
